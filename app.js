...
function ProjectCard({
  ...
  }, lastDate ? fmtD(lastDate) : "sin fin"), lastDate && parseD(lastDate) < TODAY && doneTasks < totalTasks && React.createElement("span", {
    style: {
      color: "#FF4757",
      fontSize: 8
    }
...
function PrintReport({
  ...
  }, React.createElement("span", null, "TalentCity · ", proj.projectName), React.createElement("span", null, now))));
}
function TalentCity() {
  ...
  const Dashboard = () => {
    ...
    const buildModulesFromTemplate = tipo => {
      ...
      rows.forEach(r => {
        if (!r.modulo) return;
        if (!modMap[r.modulo]) modMap[r.modulo] = {
          name: r.modulo,
          tasks: []
        };
      ...
      return Object.values(modMap).map((m, i) => ({
    ...
    const filteredProjects = myProjects.filter(({
      ...
      const matchName = !searchQuery || (proj.projectName || proj.name).toLowerCase().includes(searchQuery.toLowerCase());
    ...
    return React.createElement("div", {
      ...
      const downloadCSV = () => {
        const h = ["Proyecto", "Tipo", "Rol", "Fecha Inicio", "Fecha Fin", "Módulos Activos", "Módulos Completados", "Tareas Totales", "Tareas Realizadas", "Tareas Vencidas", "Hs Admin", "Hs Cliente", "% Avance"];
        const body = rows.map(({
          p,
          r,
          firstD,
          ...
          hoursAdmin,
          hoursTeam
        }) => [`"${p.projectName || p.name}"`, p.tipo || "—", r, firstD ? fmtD(firstD) : "—", lastD ? fmtD(lastD) : "—", am2.length, doneModules, totalTasks2, doneTasks2, overdueTasks, hoursAdmin, hoursTeam, pct2 + "%"].join(","));
      ...
      }, React.createElement("thead", null, React.createElement("tr", null, ["PROYECTO", "FECHA INICIO", "FECHA FIN", "MÓDULOS", "MOD. COMPL.", "TAREAS", "REALIZADAS", "VENCIDAS", "HS ADMIN", "HS CLIENTE", "AVANCE"].map(h2 => React.createElement("th", {
        key: h2,
        style: th
...
function ProjectApp({
  ...
  }, "PIN Admin de Proyecto ", React.createElement("span", {
    style: {
      color: "#FFD700"
    }
  }, "🏗️")), React.createElement("input", {
    style: C.inp,
    maxLength: 8,
    type: "password",
    placeholder: "1111",
  ...
  }, "PIN que usa el Admin de Proyecto al loguearse"))), React.createElement("button", {
    style: C.btn("p"),
    onClick: () => save(S)
  ...
  }, "Nombre Superadmin"), React.createElement("input", {
    style: C.inp,
  ...
  }, "PIN Superadmin ", React.createElement("span", {
    style: {
      color: "#FFD700"
    }
  }, "👑")), React.createElement("input", {
  ...
  }, "PIN del superadmin. Cambialo desde Config."))), React.createElement("div", {
    style: {
      fontSize: 10,
      color: "#7A8FA6"
    }
  }, "Para cambiar el PIN de superadmin, editá el valor y guardá el proyecto.")), React.createElement("div", {
    style: {
      border: "1px solid #FF475744",
      padding: 11
    }
