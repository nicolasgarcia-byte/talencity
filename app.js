...
function GanttCanvas({
  ...
  useEffect(() => {
    ...
    const totalRows = Math.max(am.length, 1) + hardRows.length;
    ...
    if (hardRows.length > 0) {
      ...
      hardRows.forEach((ph, i) => {
        ...
        ctx.fillStyle = "#7dc9b2";
        ctx.textAlign = "left";
        const label = ph.name.length > 14 ? ph.name.slice(0, 13) + "…" : ph.name;
        ctx.fillText(label, 22, y + rowH / 2 + 4);
        if (ph.startDate && ph.deadline) {
          const xs = xOf(ph.startDate),
            xe = Math.min(W - padR, xOf(ph.deadline));
          const barY2 = y + rowH / 2 - 5;
          if (xe > xs) {
            ctx.fillStyle = ph.isCompleted ? "#7dc9b266" : "#7dc9b233";
            ctx.fillRect(xs, barY2, xe - xs, 10);
            ctx.strokeStyle = "#7dc9b299";
            ctx.lineWidth = 1;
            ctx.strokeRect(xs, barY2, xe - xs, 10);
            if (ph.isCompleted) {
              ctx.fillStyle = "#7dc9b2CC";
              ctx.fillRect(xs, barY2, xe - xs, 10);
            }
          }
          ctx.fillStyle = ph.isCompleted ? "#A8E6CF" : "#7dc9b2";
          ctx.beginPath();
          ctx.arc(Math.min(xe, W - padR), y + rowH / 2, 5, 0, Math.PI * 2);
          ctx.fill();
        }
        const statusTxt = ph.isCompleted ? "100%" : ph.status || "—";
        ctx.font = "bold 8px 'Press Start 2P',monospace";
        ctx.fillStyle = "#7dc9b2";
        ctx.textAlign = "left";
        const labelX = ph.deadline ? Math.min(xOf(ph.deadline) + 8, W - 40) : leftW + 8;
        ctx.fillText(statusTxt, labelX, y + rowH / 2 + 4);
      });
    }
...
function PrintReport({
  ...
  return React.createElement("div", {
  ...
  }, React.createElement("span", null, "ManduHubCity · ", proj.projectName), React.createElement("span", null, now)), printHard && hardPhases.length > 0 && React.createElement("div", {
    style: {
      marginTop: 24
    }
  }, React.createElement("div", {
  ...
  }, "🔗 Fases Hard (Severa) — ", severaData?.name), React.createElement("table", {
    style: {
      width: "100%",
  ...
  }, ["FASE", "PERTENECE A", "ESTADO", "INICIO", "DEADLINE", "NOTAS", "HS"].map(h => React.createElement("th", {
    key: h,
    style: {
      padding: "5px 8px",
      textAlign: "left",
  ...
  }, h)))), React.createElement("tbody", null, hardPhases.map((ph, i) => {
    const hd = (proj.hardData || {})[ph.guid] || {
      notes: "",
      hoursLog: []
    };
    const hrs = hd.hoursLog.reduce((a, h) => a + (parseFloat(h.hours) || 0), 0);
    return React.createElement("tr", {
      ...
      style: {
        padding: "5px 8px",
        fontWeight: 600,
        color: ph.isCompleted ? "#94a3b8" : "#1e293b",
        textDecoration: ph.isCompleted ? "line-through" : "none"
      }
    }, ph.isCompleted ? "✅ " : "", ph.name), React.createElement("td", {
      style: {
        padding: "5px 8px",
        color: "#64748b"
      }
    }, ph.belongsTo || "—"), React.createElement("td", {
      style: {
        padding: "5px 8px",
        color: ph.isCompleted ? "#16a34a" : "#0369a1",
        fontWeight: 600
      }
    }, ph.isCompleted ? "Completada" : ph.status || "En curso"), React.createElement("td", {
      style: {
        padding: "5px 8px",
        color: "#64748b",
        whiteSpace: "nowrap"
      }
    }, ph.startDate ? fmtD(ph.startDate) : "—"), React.createElement("td", {
      style: {
        padding: "5px 8px",
        color: ph.isCompleted ? "#94a3b8" : parseD(ph.deadline) < TODAY ? "#dc2626" : "#64748b",
        whiteSpace: "nowrap"
      }
    }, ph.deadline ? fmtD(ph.deadline) : "—"), React.createElement("td", {
      style: {
        padding: "5px 8px",
        color: "#475569",
        maxWidth: 200
      }
    }, hd.notes || "—"), React.createElement("td", {
      style: {
        padding: "5px 8px",
        textAlign: "center",
        color: hrs > 0 ? "#0369a1" : "#94a3b8",
      ...
      try {
        ...
        clients.forEach(c => {
          ...
          if (templates.length) {
            ...
            const allTemplateMods = buildModulesFromTemplate(c.tipo || "Eshop", true);
...
function ProjectApp({
  ...
  const renderModulosView = () => {
    ...
    return React.createElement("div", {
    ...
    }, severaCache && (severaCache.phases || []).map(ph => {
      const hardData = (S.hardData || {})[ph.guid] || {
        notes: "",
        hoursLog: []
      };
      const totalHrs = hardData.hoursLog.reduce((a, h) => a + (parseFloat(h.hours) || 0), 0);
      const isLate = !ph.isCompleted && ph.deadline && parseD(ph.deadline) < TODAY;
      const showHours = expandedHard[ph.guid] || false;
      return React.createElement("div", {
        key: ph.guid,
        style: {
          ...C.card,
          borderColor: ph.isCompleted ? "#7dc9b244" : isLate ? "#FF475744" : "#7dc9b222"
        }
      }, React.createElement("div", {
        style: {
          display: "flex",
      ...
      }, ph.isCompleted ? "✅" : "🔄"), React.createElement("div", {
        style: {
          flex: 1,
          minWidth: 0
        }
      ...
      }, "↳ ", ph.belongsTo)), React.createElement("span", {
        style: {
          fontFamily: "'Press Start 2P',monospace",
          fontSize: 5,
          color: ph.isCompleted ? "#A8E6CF" : isLate ? "#FF4757" : "#7dc9b2",
          border: `1px solid ${ph.isCompleted ? "#A8E6CF44" : isLate ? "#FF475744" : "#7dc9b244"}`,
          padding: "2px 5px",
          flexShrink: 0,
          background: ph.isCompleted ? "#A8E6CF11" : isLate ? "#FF475711" : "transparent"
        }
      }, ph.isCompleted ? "HECHA" : isLate ? "VENCIDA" : "EN CURSO")), React.createElement("div", {
        style: {
          display: "flex",
          alignItems: "center",
          gap: 4,
        ...
        style: {
          ...C.dateInp,
          fontSize: 10,
          color: ph.startDate ? D.text : D.textMuted,
          padding: "3px 6px"
        }
      }, ph.startDate ? fmtD(ph.startDate) : "—"), React.createElement("span", {
        style: {
          fontSize: 9,
          color: D.textSub,
          flexShrink: 0,
        ...
        style: {
          ...C.dateInp,
          fontSize: 10,
          color: ph.isCompleted ? D.textMuted : isLate ? "#FF4757" : ph.deadline ? D.text : D.textMuted,
          padding: "3px 6px"
        }
      }, ph.deadline ? fmtD(ph.deadline) : "—")), React.createElement("div", {
        style: {
      ...
      }, "⏱ Horas: ", React.createElement("span", {
        style: {
          color: "#7dc9b2",
          fontWeight: 600
        }
      }, totalHrs > 0 ? totalHrs + "h" : "sin cargar")), React.createElement("span", {
        style: {
          fontSize: 10,
          color: D.textMuted
        }
      }, showHours ? "▲" : "▼", isEclient || isAdmin ? " Cargar" : "")), showHours && React.createElement("div", {
        style: {
          marginTop: 8
        }
      }, hardData.hoursLog.length > 0 && React.createElement("div", {
        style: {
          marginBottom: 8
        }
      }, hardData.hoursLog.slice(-5).map(h => React.createElement("div", {
        key: h.id,
        style: {
          display: "flex",
          gap: 6,
      ...
      }, h.hours, "h"), React.createElement("span", null, fmtD(h.date)))), hardData.hoursLog.length > 5 && React.createElement("div", {
        style: {
          fontSize: 9,
          color: D.textMuted
        }
      }, "...y ", hardData.hoursLog.length - 5, " más")), (isEclient || isAdmin) && React.createElement("div", {
        style: {
          display: "flex",
          gap: 6,
          alignItems: "center",
        ...
        onClick: () => {
          const dateEl = document.getElementById(`hard-date-${ph.guid}`);
          const hrsEl = document.getElementById(`hard-hrs-${ph.guid}`);
          const hrs = parseFloat(hrsEl?.value);
          if (!hrs || hrs <= 0) return;
          const entry = {
            id: "h" + Date.now(),
            date: dateEl?.value || TODAY.toISOString().split('T')[0],
            hours: hrs,
            authorId: session.id,
            authorName: session.name,
            authorEmoji: session.emoji || "🛍️"
          };
          const existing = (S.hardData || {})[ph.guid] || {
            notes: "",
            hoursLog: []
          };
          const ns = {
          ...
          _showToast("⏱ " + hrs + "h registradas en " + ph.name);
        }
      }, "+ CARGAR")))), React.createElement("div", {
        style: {
          borderTop: `1px solid ${D.cardBorder}`,
          paddingTop: 6,
          marginTop: 6
        ...
        placeholder: "Escribí una nota sobre esta fase...",
        value: hardData.notes || "",
        onChange: e => {
          const ns = {
            ...S,
            hardData: {
    ...
    })), severaCache && (!severaCache.phases || severaCache.phases.length === 0) && React.createElement("div", {
      style: {
        ...C.card,
        color: D.textMuted,
        fontSize: 11
