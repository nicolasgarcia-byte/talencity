...
function ProjectApp({
  ...
  const renderConfigView = () => {
    ...
    return React.createElement("div", {
      ...
      type: "password",
      maxLength: 8,
      placeholder: "PIN actual: 0000",
      defaultValue: appState.superadminPin || "0000",
      id: "superadmin-pin-input"
    }), React.createElement("button", {
      style: C.btn("p"),
      onClick: () => {
        const el = document.getElementById("superadmin-pin-input");
        if (!el || !el.value.trim()) return;
        const ns = {
          ...appState,
          superadminPin: el.value.trim()
        };
        updApp(ns);
        persist2(ns);
        _showToast("👑 PIN de superadmin actualizado");
      }
    ...
    }, "Mínimo 4 caracteres. Todos los superadmins usan el mismo PIN.")), React.createElement("div", {
      style: {
        fontSize: 11,
        color: D.textSub,
        marginBottom: 6
      }
    }, "EMAILS CON ACCESO"), superAdmins.map((email, i) => React.createElement("div", {
      key: i,
      style: {
        display: "flex",
        gap: 6,
      ...
      onClick: () => {
        updSuperAdmins([...superAdmins, ""]);
        setTimeout(() => {
          const el = document.getElementById(`sa-email-${superAdmins.length}`);
          if (el) el.focus();
        }, 50);
      }
