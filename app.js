...
window.storage = {
  get: async function(key) {
    try {
      const url = SCRIPT_URL + "?key=" + encodeURIComponent(key) + "&t=" + Date.now();
  ...
  set: async function(key, value) {
    try {
      ...
      const data = await res.json();
      if (data && data.error) { console.error("storage.set error:", data.error); return null; }
      return data;
...
function ProjectCard({
  ...
  }, proj.team.length, " ciudadano", proj.team.length !== 1 ? "s" : "", " · ", am.length, " módulo", am.length !== 1 ? "s" : "")), React.createElement("div", {
    style: {
      textAlign: "right",
      flexShrink: 0
    }
...
function TalentCity() {
  ...
  const Dashboard = () => {
    ...
    }, "CANCELAR"))), myProjects.length === 0 ? React.createElement("div", {
      style: {
        background: "#1A2332",
    ...
    }, "Contactá al administrador para que te asigne a un proyecto.")) : React.createElement("div", {
      style: {
        display: "grid",
